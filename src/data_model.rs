use crate::{
    model_config,
    vad_error::{ConfigErr, ModelHandelErr, ParseErr, TypeConvertErr},
};
use ndarray::Array1;
...
impl ModelHandeler {
        ...
        It: int::PrimInt + FromPrimitive + Zero + std::iter::Sum,
    {
        Ok(Self {
            sr: match cfg.orig_sr.clone().to_u32() {
                Some(v) => v,
                None => return Err(Box::new(TypeConvertErr::new())),
            },
            ...
            + ToPrimitive,
    {
        let result = self
            .model
            .process_chunk(&tsk.0.view(), self.sr)
            .map_err(|_| {
                Box::new(ModelHandelErr::new()) as Box<dyn std::error::Error + Send + Sync>
            })?;
        ...
        Ft::from_f32(1. - max_value).ok_or_else(|| {
